...
def gs_login(usr, psw, project, log_filename):
    ...
    first_try = True
    while (
        re.search('name="csrfmiddlewaretoken" value="(.+)"', response.text) is not None
        or first_try
    ):
        if not first_try:
...
def download_filenames(project, cookies, log_filename):
    ...
    response = requests.get(
        host + "data_api2/" + str(project) + "/n", cookies=cookies, params={"cd": "."}
    )
    try:
        ...
        log_entry(
            "[GS get filelist] Error reading response:" + response.text,
            True,
            log_filename,
        )
        exit(1)
