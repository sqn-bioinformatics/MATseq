...
def removedups_with_loading(R1_path,R2_path,R3_path,sample,run):
    ...
    for i in range(ninitial):
        mergedreads.append(R1_reads[i*4+1][25:50]+R3_reads[i*4+1][25:50]+R2_reads[i*4+1])
    ...
    del mergedreads #clears memory
    del uniquereads #clears memory
    ...
    del R1_reads #clears memory
    del R2_reads #clears memory
    del R3_reads #clears memory
...
def change_header(fastq,run): #adds run info to each read header
...
def removedups_NO_loading(ddR1,ddR2,ddR3,sample,run):
    ...
    mergedreads=[]
    for i in range(ninitial):
        mergedreads.append(ddR1[i*4+1][25:50]+ddR3[i*4+1][25:50]+ddR2[i*4+1])
    log_entry('pairs created',True,log_filename)
...
for sample in samplelist:
    ...
    log_entry(str(len(runlist))+' runs found for sample '+sample,True,log_filename)
    
    for run in runlist: #iterates every run to remove duplicates
      ...
      (ddR1_temp,ddR2_temp,ddR3_temp,ninitial,nfinal)=removedups_with_loading(R1_path,R2_path,R3_path,sample,run)
      gc.collect()
    ...
    if len(runlist)>1:
      (ddR1,ddR2,ddR3,ninitial,nfinal)=removedups_NO_loading(ddR1,ddR2,ddR3,sample,'merged')
